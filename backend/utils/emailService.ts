...
const verifyEmailMail = async (email: string, name: string, link: string) => {
    const {data, error} = await resend.emails.send({
        ...
        html: fs.readFileSync('../templates/verify-email.html', 'utf8').replace("{{name}}", name).replace("{{magic_link}}", link)
    })
...
const forgetPasswordMail = async (email: string, name: string, link: string) => {
    const {data, error} = await resend.emails.send({
        ...
        html: fs.readFileSync("../templates/forgot-password.html", 'utf-8').replace("{{name}}", name).replace("{{magic_link}}", link)
    })
